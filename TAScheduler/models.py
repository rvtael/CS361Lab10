...
class Lab(models.Model):
    ...
    instructor = models.ForeignKey(UserProfile.userID, on_delete=models.PROTECT)
    TA = models.ForeignKey(UserProfile.userID, on_delete=models.PROTECT)
...
class Course(models.Model):
    ...
    days = models.CharField(max_length=20)
    instructor = models.ForeignKey(UserProfile.userID, on_delete=models.PROTECT)
    TAs = models.ManyToManyField(UserProfile.userID)
    labs = models.ManyToManyField(Lab.labID)


# Schedule is a tool to be used by users to display the events they have going on in a week, be that Labs or courses.
class Schedule(models.Model):
    userID = models.ForeignKey(UserProfile.userID, on_delete=models.CASCADE)
    courses = models.ManyToManyField(Course.courseID)
    labs = models.ManyToManyField(Lab.labID)
